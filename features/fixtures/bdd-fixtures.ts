...
// Define the same fixture type as in base_page_fixtures
type CryptoComPageFixture = {
  navigateToExchange: () => Promise<void>;
  getNavigationItem: (itemText: string) => import('@playwright/test').Locator;
...
// Extend playwright-bdd test with our fixtures (same as base_page_fixtures.ts)
export const test = base.extend<CryptoComPageFixture>({
  ...
  getNavigationItem: async ({ page }, use) => {
    const getNavItem = (itemText: string) => {
      const selectors = [
        `text="${itemText}"`,
        `*:has-text("${itemText}")`,
